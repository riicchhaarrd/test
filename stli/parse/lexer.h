...
#define TOKEN_FLAG_NONE (0)
#define TOKEN_FLAG_DECIMAL_POINT (1)
#define TOKEN_FLAG_HEXADECIMAL (2)
...
{
	Stream *stream;
	jmp_buf jmp_error; // Probably would've been better to make jmp_error a pointer and make it NULL by default
	int flags;
	FILE *out;
	void *userptr;
} Lexer;
...
LEXER_STATIC Token *lexer_read_characters(Lexer *lexer, Token *t, TokenType token_type, int (*cond)(Token *t, u8 ch, int *undo))
{
	// https://en.wikipedia.org/wiki/Fowler%E2%80%93Noll%E2%80%93Vo_hash_function
	u64 prime = 0x00000100000001B3;
	u64 offset = 0xcbf29ce484222325;
...
LEXER_STATIC int cond_string(Token *t, u8 ch, int *undo)
{
	*undo = 0;
	return ch == '"';
}
LEXER_STATIC int cond_numeric(Token *t, u8 ch, int *undo)
{
	*undo = 1;
	...
	if(t->flags & TOKEN_FLAG_HEXADECIMAL)
	{
	...
	} else
	{
		switch(ch)
		{
			...
			case 'x': t->flags |= TOKEN_FLAG_HEXADECIMAL; return 0;
			// Floating point and 'f' postfix
			case '.':
			case 'f': t->flags |= TOKEN_FLAG_DECIMAL_POINT; return 0;
		}
	}
	return 1;
}
LEXER_STATIC int cond_ident(Token *t, u8 ch, int *undo)
{
	*undo = 1;
	return !(ch >= 'a' && ch <= 'z') && !(ch >= 'A' && ch <= 'Z') && ch != '_' && !(ch >= '0' && ch <= '9');
}
LEXER_STATIC int cond_single_line_comment(Token *t, u8 ch, int *undo)
{
	*undo = 1;
	//\0 is implicitly handled by the if(!ch) check in lexer_read_characters
	return ch == '\r' || ch == '\n';
}
LEXER_STATIC int cond_whitespace(Token *t, u8 ch, int *undo)
{
	*undo = 1;
	return ch == '\r' || ch == '\n' || ch == ' ' || ch == '\t';
}
...
repeat:
	...
	switch(ch)
	{
		...
		case '-':
		{
			if(lexer->flags & LEXER_FLAG_TREAT_NEGATIVE_SIGN_AS_NUMBER)
			{
				lexer_unget(lexer);
		...
		default:
		{
			...
			{
				// Commented out because lexer_error would longjmp
				// if(ch >= 0x20 && ch <= 0x7e)
				// if(!(ch >= 0x20 && ch <= 0xff))
				// {
				// 	fprintf(lexer->out, "%d\n", ch);
...
LEXER_STATIC void lexer_expect_read_string(Lexer *l, TokenType tt, char *str, size_t max_str)
{
	Token t;
	lexer_expect(l, tt, &t);
...
LEXER_STATIC void lexer_any(Lexer *l, char *str, size_t max_str)
{
	Token t;
	lexer_step(l, &t);
	lexer_token_read_string(l, &t, str, max_str);

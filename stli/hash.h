...
static uint64_t fnv1a_64_lc(const char *str)
{
	uint64_t prime = 0x00000100000001B3;
	uint64_t offset = 0xcbf29ce484222325;
	...
	while(*str)
	{
		hash ^= (unsigned char)tolower(*str);
		hash *= prime;
		++str;
	}
	return hash;
...
static uint64_t fnv1a_64_filter(const char *str, unsigned char (*filter)(unsigned char))
{
	uint64_t prime = 0x00000100000001B3;
	uint64_t offset = 0xcbf29ce484222325;
